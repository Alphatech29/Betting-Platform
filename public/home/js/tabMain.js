(function () {
  function TabbedNavigation(element) {
    ...
    this.navigationElements =
      this.navigation.getElementsByClassName("js-cd-navigation")[0];
  ...
  TabbedNavigation.prototype.init = function () {
    ...
    this.navigation.addEventListener("click", function (event) {
      ...
      if (
        ...
        !hasClass(event.target, "cd-selected")
      ) {
        self.activeTab = event.target;
        self.activeContent = self.content.querySelectorAll(
          "[data-content=" + self.activeTab.getAttribute("data-content") + "]"
        )[0];
        self.updateContent();
      }
    });
  ...
  TabbedNavigation.prototype.updateContent = function () {
    ...
    removeClass(
      this.navigation.querySelectorAll("a.cd-selected")[0],
      "cd-selected"
    );
    addClass(this.activeTab, "cd-selected");
    removeClass(
      this.content.querySelectorAll("li.cd-selected")[0],
      "cd-selected"
    );
    ...
    !window.requestAnimationFrame
      ? this.content.setAttribute(
          "style",
          "height:" + this.activeContent.offsetHeight + "px;"
        )
      : setHeight(
          actualHeight,
  ...
  TabbedNavigation.prototype.toggleNavShadow = function () {
    ...
    var navigationWidth = Math.floor(
        this.navigationElements.getBoundingClientRect().width
      ),
      navigationViewport = Math.ceil(
        this.navigation.getBoundingClientRect().width
      );
